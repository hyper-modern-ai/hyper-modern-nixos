...
in {
  ...
  config = mkIf cfg.enable {
    ...
    environment.variables.AWS_CREDENTIAL_FILE =
      /secrets/absolution-gap-api-token; # config.deployment.keys.s3fs-api-token.path;
    ...
    systemd.user.services.s3fs = {
      ...
      serviceConfig = {
        ...
        ExecStart =
          "${pkgs.s3fs.outPath}/bin/s3fs absolution-gap /home/${config.primary-user.name}/mnt -o url=https://sfo3.digitaloceanspaces.com -o use_path_request_style -o passwd_file=/secrets/s3fs-api-token -o endpoint=sfo3";

{
  ...
  outputs = inputs@{ self, nixpkgs, unstable, flake-utils, home-manager, disko
    , sops-nix, nixos-hardware, nixos-generators, kmonad, brightness-bar
    , volume-bar, xmonad-solomon, }:
    let
      system = "x86_64-linux";
      pkgs = import nixpkgs {
        inherit system;
    ...
    in {
      ...
      packages.x86_64-linux = {
        ...
        nixos-iso = nixos-generators.nixosGenerate {
          inherit system;
          format = "install-iso";
          modules = [ ./installer home-manager.nixosModules.home-manager ];
        };
        ...
        in pkgs.symlinkJoin {
          ...
          postBuild =
            "wrapProgram $out/bin/install-pc --prefix PATH : $out/bin";
        };
        ...
        install-server = let
          ...
          script = (pkgs.writeScriptBin "install-server" src).overrideAttrs
            (old: {
              buildCommand = ''
                ${old.buildCommand}
                 patchShebangs $out'';
        ...
        in pkgs.symlinkJoin {
          ...
          buildInputs = [ pkgs.makeWrapper ];
          postBuild =
            "wrapProgram $out/bin/install-server --prefix PATH : $out/bin";
        };
      ...
      apps = {
        deploy =
          flake-utils.lib.mkApp { drv = self.packages.${system}.deploy; };
        deploy-all =
          flake-utils.lib.mkApp { drv = self.packages.${system}.deploy-all; };
      ...
      (builtins.readDir ./config/machines/servers) // builtins.mapAttrs
